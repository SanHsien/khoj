...
> **This is a Windows-first maintenance fork of [`khoj-ai/khoj`](https://github.com/khoj-ai/khoj).** It keeps the GNU AGPL v3.0 license and full git history. Product behaviour follows upstream; this line adds Traditional Chinese docs, a Windows development gate, and commit-by-commit upstream review. See [`FORK.md`](FORK.md) and [`docs/UPSTREAM.md`](docs/UPSTREAM.md). Official Docker images and PyPI packages still come from upstream. This fork does not publish to PyPI, GHCR, docs.khoj.dev, or the Obsidian plugin store.
...
To get started with self-hosting Khoj, [read the docs](https://docs.khoj.dev/get-started/setup). On Windows this fork recommends Docker:
...
The app listens on http://127.0.0.1:42110. Official images remain `ghcr.io/khoj-ai/khoj`. This fork's Compose binds that port to loopback, disables `--anonymous-mode`, and sets `KHOJ_TELEMETRY_DISABLE=True`. The computer/VNC service starts only with `docker compose --profile operator up`. Change the sample admin password and `KHOJ_DJANGO_SECRET_KEY` before exposing the port beyond localhost.
